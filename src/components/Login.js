...
// import { router } from "next/router"; // <-- pages/ router. If using app/, use: from "next/navigation"
...
function Login() {
  ...
  function handleErrors(formValues) {
    const nextErrors = {};
    const emailOk = /([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9._-]+)/.test(
      formValues.email || ""
    );
  ...
  useEffect(() => {
    ...
    if (Object.keys(errors).length === 0) {
      axios
        ...
        .then((res) => {
          const { user_id, industry } = res?.data?.userInfo || {};
          setDataBaseMsg({ msg: "Successfully Logged In.", color: "success" });

          // Note: "Secure" cookies won't set on http://localhost
          document.cookie = `user_id=${encodeURIComponent(user_id)}; path=/; SameSite=Strict`;
        ...
        .catch((err) => {
          ...
          setDataBaseMsg({ msg, color: "error" });
          setSubmitting(false);
        });
    } else {
      // validation errors present -> stop submitting so user can fix inputs
      setSubmitting(false);
    }
  }, [errors, submitting, loginData, navigate]);
    ...
    <div className="low-padding-container">
      ...
      <div className="flex justify-content-center">
        <div className="card flex justify-content-center" style={{ width: "30vw" }}>
          ...
          {dataBaseMsg.msg && (
            <Message
              severity={dataBaseMsg.color}
              text={dataBaseMsg.msg}
